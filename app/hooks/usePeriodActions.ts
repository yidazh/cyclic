...
import { periodManager, breakReminderService } from '@/services';
...
export function usePeriodActions() {
  ...
  const updateActivePeriod = useCallback(
    async (updates: { name?: string; notes?: string; theme?: string; category?: string; tags?: string[] }) => {
      const activePeriod = useStore.getState().activePeriod;
      if (!activePeriod) return;

      try {
        const updated = await periodManager.updatePeriod(activePeriod.id, updates);
