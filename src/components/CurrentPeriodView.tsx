...
import { breakReminderService, timerManager } from "@/services";
...
export function CurrentPeriodView() {
	...
	const { transitionPeriod, pauseResume, updateActivePeriod } =
		usePeriodActions();
	...
	useEffect(() => {
		...
		// Immediately calculate elapsed time from period's startTime (source of truth)
		// This avoids race condition with useTimerSubscription hook
	...
	const handleStartReminder = async () => {
		if (reminderActive) {
			...
			const permission =
				await breakReminderService.requestNotificationPermission();

			if (permission === "granted") {
				breakReminderService.startReminder(
					config.settings.reminder.defaultDuration,
				);
